...
// skip to just past the delimiter c - NO ERROR CHECKING!
char *skip_char(char *s,char c){
  while(*s++ != c);
  return(s);
}

// get our lat/lon and GMT offset from a web service (ip-api.com)
// call returns {"success||fail", lat, long, offsetInSeconds}
void init_location(struct location *l){
  ...
  result = pipe(the_pipe);
  if(result < 0) {printf("failed to create pipe\n"); exit(1);}
  
  process_id = fork();
  if(process_id == 0){    //we're the child
    close(the_pipe[0]);   // close the input side of the pipe
    dup2(the_pipe[1],1);  // make the writeable side of the pipe STDOUT for this process
    char *args[] = {"/usr/bin/curl", "http://ip-api.com/csv/?fields=status,lat,lon,offset", NULL}; 
    execv("/usr/bin/curl", args);     // do the lookup (we could use libcurl for this, but it ain't broke, so...)

    close(the_pipe[0]);     // fd no longer needed - the dup'ed handles are sufficient
    //close(err_out);     // fd no longer needed - the dup'ed handles are sufficient
    exit(0);
  }
  else {  // we're the parent:
    waitpid(process_id,&x,0);
    result = read(the_pipe[0],inbuf,128);
    inbuf[result]=0;
    if(result < 0) {printf("failed to read pipe. error %d\n",result); exit(1);}
  }
  
  scan_ptr = skip_char(scan_ptr,',');  // skip first argument
  lat = atof(scan_ptr);

  scan_ptr = skip_char(scan_ptr,',');  // skip to the next argument
  lon = atof(scan_ptr);

  scan_ptr = skip_char(scan_ptr,',');  // next arg
  offset = atoi(scan_ptr);

  printf("lat = %f, long = %f, offset from GMT = %d\n", lat, lon, offset);
  

  l->latitude = lat;		

  l->longitude = -lon;    // NOTE: sign needs to be negated.  We consider West to be positive

  l->viewing_date = time(NULL);
  l->gmt_offset = offset;
  printf("lat = %f, long = %f, offset from GMT = %d\n", lat, lon, l->gmt_offset);
  l->initialized=1;
}
...
void calcLunarAzimuth(double *azimuthResult, double *elevationResult, double *fullness, double *rightAscensionResult, double *declinationResult, time_t theDate,struct location theLocation){
  ...
  // Moon's mean longitude, including constant term for the effec
...
time_t calcSunOrMoonRiseForDate(time_t the_date, int oneForRiseTwoForSet, int oneForSunTwoForMoon,struct location theLocation){
  ...
  for (iteration = 0; iteration<2; iteration++) {
	...
	m[2] = m[0] + H0/360; //moonset as fraction of 24 hrs
      ...
      double lagRelativeToGMT = -(double) (theLocation.gmt_offset) / 86400.0;
...
int main(){
  ...
  for(som=1;som<3;som++)   
    for(ros=1;ros<3;ros++){
      ...
      event_time += myLocation.gmt_offset;
